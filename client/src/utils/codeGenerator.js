import { branchAwareSort } from "./pipelineValidator";
...
export const generateCode = (blocks, trainingSettings = DEFAULT_SETTINGS) => {
  ...
  const groups = branchAwareSort(blocks);
  ...
  let needsModelSep = false;

  for (const group of groups) {
    const mlStart = modelLines.length;
    ...
    if (modelLines.length > mlStart) {
      if (needsModelSep) {
        modelLines.splice(mlStart, 0, "");
      }
      needsModelSep = true;
    }
  }
